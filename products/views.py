...
def new_sku(request):
    sku_brand = SkuBrand.objects.all()
    if request.method == "POST":
        print("ww")
        form = SkuForm(request.POST, request.FILES)

        if form.is_valid():
            new_sku = form.save(commit=False)
            if request.POST.get('n_brand'):
                new_sku.brand = sku_brand.get(name= request.POST.get('n_brand'))
            
            if request.POST.get('s_notes'):
                new_sku.notes = request.POST.get('s_notes')
